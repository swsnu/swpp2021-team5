...
import * as actionCreatorsMenu from '../../store/actions/Menu/menu';
import * as actionCreatorsUser from '../../store/actions/User/user';
...
const menuInitialState = {
  ...
  allMenus: null,
  recommendedMenus: [
    {"id": 3970, "name": "1927 peanut butter bread", "calories": 173.9, "carbs": 8.0, "protein": 12.0, "fat": 8.0, "image": "http://localhost:8000/media/1927-peanut-butter-bread-450534.jpg", "recipe": "['1. preheat oven to 350 degrees fahrenheit', '2. sift first 4 ingredients together in bowl', '3. add pb , mix well', '4. add milk , beat well', '5. pour batter into a loaf tin', '6. bake 1 hour or until a toothpick comes out clean']", "ingredient": "['flour', 'baking powder', 'salt', 'sugar', 'peanut butter', 'skim milk']"}, 
...
describe('<Main />', () => {
  let spyGetUserSetting;
  let spyGetRecommendedMenus;
  let spyGetCountAll;
  let spyGetUserNutrition;
  beforeEach(() => {
    main = (
    ...
    spyGetUserSetting = jest.spyOn(actionCreatorsUser, 'getUserSetting')
      .mockImplementation(() => (dispatch) => {});
    spyGetRecommendedMenus = jest.spyOn(actionCreatorsMenu, 'getRecommendedMenus')
      .mockImplementation(() => (dispatch) => {});
    spyGetCountAll = jest.spyOn(actionCreatorsMenu, 'getCountAll')
      .mockImplementation(() => (dispatch) => {});
    spyGetUserNutrition = jest.spyOn(actionCreatorsUser, 'getUserNutrition')
      .mockImplementation(() => (dispatch) => {});
  });
  ...
  it('should render without errors', () => {
    ...
    expect(spyGetUserNutrition).toBeCalled();
    expect(spyGetRecommendedMenus).toBeCalled();
    expect(spyGetCountAll).toBeCalled();
    expect(spyGetUserSetting).toBeCalled();
  });
  ...
  xit('should handle followed recommendation button click', () => {
    ...
    component.find('icon#followed-recommendation-button').simulate('click');
    expect(history.location.pathname).toBe('/record');
    component.unmount();
  });
